...
[issue](https://github.com/waynehoggett/Personal-AI-Router/issues).
...
| Linux with an AMD APU (integrated Radeon graphics) | **Understated.** PAIR reports the BIOS VRAM carve-out the `amdgpu` driver exposes, not the system memory the GPU can also draw on. Discrete AMD cards report correctly. |
| Linux with Intel graphics, or an AMD card on the legacy `radeon` driver | **Missing.** PAIR lists the GPU by name with no memory figure and no utilization. |
...
The other three cases are the gap.
...
## Tailscale Nodes Must Be Added by IP

A node on another network reached over Tailscale is not discovered; add it from
**Add node** with **Node is on another network** selected, as described in
[Nodes on Other Networks](getting-started.mdx#nodes-on-other-networks-tailscale).
Enter its Tailscale IP, not its MagicDNS name: the cluster resolves paired peers
by IP, so a hostname pairs but is not re-resolved afterwards and the node can
drop out of the routing pool. Tailscale IPs are stable per device, so this
costs nothing in practice.

## Scheduling Only Counts Jobs

The single scheduling policy ranks nodes by how much work is already queued on
them and coarse GPU pressure. It does not consider GPU model, available memory,
measured latency, whether a model is already loaded, or how expensive a request
looks. Latency is now measured and shown on each node's card, but it does not
yet steer routing, so a request can land on a distant node over Tailscale as
readily as on one beside you. On a cluster of similar machines this behaves reasonably. On a mixed one,
