...
### Automatic Debian Package Builds

Every push to `main` runs the **Build Debian Package**
...
and the existing `v*` tags, builds a `.deb` for both `x64` and `arm64` stamped
with that version, and publishes both as a GitHub release named for the tag.
The workflow can also be started by hand from the **Actions** tab.

Download the package for your architecture from the repository's Releases page
and install it from the directory you downloaded it into:
...
version in `desktop/package.json` by hand to start a new release series.
