...
(`.github/workflows/build-deb.yml`) builds a `.deb` for `x64` and `arm64` on
every push to `main`, then tags and publishes a
[GitHub release](https://github.com/NVIDIA/Personal-AI-Router/releases) with
both packages attached. It exists so testers (for example, verifying GPU
vendor support such as AMD) can grab an installable package without setting up
the [Prerequisites](#prerequisites) locally. Install the downloaded package
with:

```bash
sudo dpkg -i personal-ai-router_*.deb
```
