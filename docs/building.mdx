...
[GitHub Actions workflow](../.github/workflows/build-deb.yml), which builds a
`.deb` for both `x64` and `arm64`, tags a new patch version, and publishes both
packages to the
[GitHub releases page](https://github.com/NVIDIA/Personal-AI-Router/releases).
Download the architecture that matches your machine and install it with:

```bash
sudo dpkg -i personal-ai-router-*.deb
```

This is the quickest way to get a build for hardware you cannot compile on
directly, such as testing engine integrations on an AMD GPU machine, without
installing the Node and Go toolchain locally.
