...
models, and routes to it like any discovered node. The Windows installer's
firewall rules allow the Tailscale range (`100.64.0.0/10`) alongside the local
subnet, so no firewall change is needed there; on Linux, allow the ports above
from the `tailscale0` interface if you run a restrictive firewall.
