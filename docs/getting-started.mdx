...
[PAIR releases page](https://github.com/waynehoggett/Personal-AI-Router/releases).
...
### Nodes on Other Networks (Tailscale)

Discovery uses multicast, which does not cross a router or a VPN, so a node on
another network is never listed automatically. A node reached over
[Tailscale](https://tailscale.com/) works if you add it by address:

1. Install Tailscale on both machines and sign them into the same tailnet.
2. On the inviting machine, open **Add node**, enter the other node's Tailscale
   IP (a `100.x.y.z` address, shown by `tailscale ip -4`), select **Node is on
   another network**, then **Invite**.
3. Enter the PIN on the other node as usual.

PAIR keeps that address as a manual node, polls it directly for telemetry and
models, and routes to it like any discovered node.

#### Limiting Tailnet Traffic to PAIR

A tailnet peer needs only the ports in the table above except `5353/udp`
(there is no multicast on a tailnet), plus the proxy port each node serves
inference on (`11434` for Ollama, `1234` for LM Studio, unless changed). PAIR
opens nothing else, and holds the line below the firewall too: the proxies
accept plaintext only from loopback and require the cluster's mutual TLS from
any other address, so a tailnet device that is not a paired member can read a
node's inventory but cannot run inference on it.

- **Windows.** The installer adds a separate set of Tailscale rules that admit
  only PAIR's own services, and only for packets addressed to the host's
  Tailscale address, which can only arrive through Tailscale's authenticated
  tunnel. No change is needed.
- **Linux.** Allow those ports on the `tailscale0` interface only, for example
  with `ufw`:

  ```bash
  sudo ufw allow in on tailscale0 to any port 14318:14323 proto tcp
  sudo ufw allow in on tailscale0 to any port 11434 proto tcp
  ```

- **Tailscale ACLs.** Tighten this again on the tailnet itself so only your
  PAIR machines can reach those ports on each other. Tag each PAIR node and
  allow only those ports between the tag:

  ```json
  {
    "tagOwners": { "tag:pair": ["autogroup:admin"] },
    "acls": [
      {
        "action": "accept",
        "src": ["tag:pair"],
        "dst": ["tag:pair:14318-14323", "tag:pair:11434", "tag:pair:1234"]
      }
    ]
  }
  ```

  With that policy, every other device on the tailnet, and every other port on
  the PAIR machines, is unreachable across it. Add the proxy or engine ports you
  have changed in **Engine settings** to the `dst` list.

Use the Tailscale IP rather than the MagicDNS name. Tailscale IPs are stable per
device, and PAIR's cluster membership resolves peers by IP. Inference over a
tailnet is bounded by the link between the two sites: a request served on a
remote node pays that round trip on every token, so keep latency-sensitive
work on the local network where you can. The node's card shows the measured
round trip from this machine (for example `23 ms`) once PAIR has polled it, so
you can see that cost before routing to it.
