...
- `vram_bytes` is reported through DXGI on Windows, `nvidia-smi` (NVIDIA) or the `amdgpu` driver's sysfs attributes (AMD) on Linux, and IORegistry on macOS. On a unified-memory NVIDIA GPU such as DGX Spark, Linux uses total physical system memory for `vram_bytes` and the independently sampled system-memory usage for `vram_used_bytes`. On an AMD APU, Linux reports the BIOS VRAM carve-out, not the system memory the GPU can also map, so an integrated AMD GPU reads understated. On Apple Silicon, `vram_bytes` is total physical unified memory and `vram_used_bytes` is the GPU driver's mapped allocation (`Alloc system memory`), not whole-system RAM usage or the momentarily active subset.
...
- **Linux** (first-class): NVIDIA GPU inventory, dedicated VRAM usage, and utilization come from `nvidia-smi`; AMD GPU inventory, VRAM, and utilization come from the `amdgpu` kernel driver's sysfs attributes (`/sys/class/drm/card*/device/{mem_info_vram_total,mem_info_vram_used,gpu_busy_percent}`), so no ROCm install is needed and a host may carry both vendors at once. CPU and system-memory usage come from `/proc`. Unified-memory NVIDIA GPUs use the `/proc/meminfo` system-memory snapshot even when dynamic `nvidia-smi` collection is unavailable. Adapters from neither source (Intel, or an AMD card on the legacy `radeon` driver) fall back to names from `ghw` without dynamic GPU stats.
